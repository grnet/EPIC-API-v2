  ...
  class Profile < Resource
    ...
    class GRNETPID < Profile
      ...
      def self.update( request, prefix, suffix, old_values, new_values )
        ...
        # React on failed inst_checks
        unless inst_check_passed
          message = "Enforcing GRNET-Profile. Handle update blocked, as the #{username} requested a handle with a missmatching Institute-Code"
          LOGGER.info(message)
          raise Rackful::HTTP403Forbidden, message
        end
        new_values
      ...
      def self.checkUserEntry(request)
        # Check if a insitute code is available in the config
        if USERS[request.env['REMOTE_USER']][:institute].nil?
          message = "Enforcing GRNET-Profile. No Insitute-Code set for user #{request.env['REMOTE_USER']}. Request blocked with Error 403 - Forbidden."
          LOGGER.warn(message)
          raise Rackful::HTTP403Forbidden, message
        end
      end
      ...
      def self.enforce_inst_record (values, inst_number)
        ...
        inst_record.parsed_data = inst_number
        values << inst_record
        LOGGER.info("Enforcing GRNET-Profile. Institute-Code #{inst_number} appended to Handle.")
        values
      end


    end # class GRNETPID < Profile
